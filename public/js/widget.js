/**
 * WhatsYpzck Chat Widget — embeddable script
...
(function () {
  ...
  if (!widgetId) { console.warn('[WhatsYpzck Widget] Missing data-widget-id attribute'); return; }
  ...
  function init() {
    ...
    fetchConfig(function (err, config) {
      if (err) { console.warn('[WhatsYpzck Widget] Failed to load config:', err); return; }
      cfg = config;
      if (cfg.whatsappMode) {
        buildWaButton(cfg);
      } else {
