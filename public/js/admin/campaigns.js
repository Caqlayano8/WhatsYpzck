  ...
  if (!list.length) {
    tbody.innerHTML = `
      <tr><td colspan="7" class="px-6 py-16 text-center">
        ...
        <div class="text-sm font-medium text-gray-500 mb-1">Henüz kampanya yok</div>
        <button onclick="openCampaignModal()" class="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl font-semibold mt-2">Yeni Kampanya</button>
      </td></tr>`;
    return;
  }
  list.forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td class="px-5 py-3">
        <div class="flex items-center gap-1 flex-wrap">
          <button onclick="viewCampaign('${c._id}')" title="Görüntüle"
          ...
          <button onclick="viewDeliveryReport('${c._id}')" title="Teslimat Raporu"
          ...
          <button onclick="pauseCampaign('${c._id}')" title="Duraklat"
          ...
          ${c.status === 'paused' ? `
          <button onclick="resumeCampaign('${c._id}')" title="Devam Et"
          ...
          <button onclick="cancelCampaign('${c._id}')" title="İptal"
          ...
          <button onclick="archiveCampaign('${c._id}')" title="Arşivle"
          ...
          <button onclick="duplicateCampaign('${c._id}')" title="Çoğalt"
          ...
          <button onclick="deleteCampaign('${c._id}')" title="Sil"
...
window.openCampaignModal = function (prefill = null) {
  const AS = window.AdminState;
  document.getElementById('campaign-modal-title').textContent = prefill ? 'Kampanyayı Çoğalt' : 'Yeni Kampanya';
...
window.advanceCampaignTab = function (direction) {
  ...
  if (direction > 0) {
    if (AS.currentCampaignTab === 'compose') {
      if (!document.getElementById('campaign-name').value.trim())    { showToast('Lütfen bir kampanya adı girin', 'warning'); return; }
      if (!document.getElementById('campaign-message').value.trim()) { showToast('Lütfen bir mesaj yazın', 'warning'); return; }
    } else if (AS.currentCampaignTab === 'recipients') {
      if (!document.querySelectorAll('#selected-contacts-list .sel-chip').length) {
        showToast('Lütfen en az bir alıcı seçin', 'warning'); return;
      }
    }
...
window.submitCampaign = async function () {
  ...
  if (!name || !message) { showToast('Ad ve mesaj zorunludur', 'warning'); return; }
  if (!contacts.length)  { showToast('En az bir alıcı seçin', 'warning'); return; }
  ...
  try {
    ...
    showToast(scheduledAt ? 'Kampanya zamanlandı!' : 'Kampanya gönderildi!', 'success');
    closeCampaignModal();
    loadCampaigns();
  } catch {
...
window.duplicateCampaign = function (id) {
  ...
  if (!c) return;
  openCampaignModal({ name: c.name + ' (kopya)', message: c.message, contacts: c.contacts || [] });
};
  ...
  document.getElementById('view-campaign-body').innerHTML = `
    <div class="flex items-start justify-between mb-4">
      ...
      <div class="text-right text-xs text-gray-400">
        <div>Oluşturuldu: ${fmtDate(c.createdAt)}</div>
        ${c.scheduledAt ? `<div>Zamanlandı: ${fmtDate(c.scheduledAt)}</div>` : ''}
      </div>
    </div>
    <div class="grid grid-cols-3 gap-3 mb-4">
      <div class="bg-gray-50 rounded-xl p-3 text-center"><div class="text-xl font-bold text-gray-900">${(c.contacts || []).length}</div><div class="text-xs text-gray-500 mt-0.5">Alıcılar</div></div>
      <div class="bg-green-50 rounded-xl p-3 text-center"><div class="text-xl font-bold text-green-600">${c.sentCount || 0}</div><div class="text-xs text-gray-500 mt-0.5">Gönderildi</div></div>
      <div class="bg-red-50 rounded-xl p-3 text-center"><div class="text-xl font-bold text-red-500">${c.failedCount || 0}</div><div class="text-xs text-gray-500 mt-0.5">Başarısız</div></div>
    </div>
    ${(c.recurringType && c.recurringType !== 'none') ? `<div class="text-xs bg-blue-50 text-blue-700 rounded-lg p-2 mb-3 font-medium"><i class="fas fa-redo mr-1"></i>Tekrarlayan: ${c.recurringType}</div>` : ''}
    <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Mesaj</div>
    ...
    <div class="flex justify-end gap-2 mt-4 pt-4 border-t border-gray-100">
      <button onclick="closeCampaignViewModal();duplicateCampaign('${c._id}')" class="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-xl font-semibold">Çoğalt</button>
      <button onclick="closeCampaignViewModal()" class="text-sm bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl font-semibold">Kapat</button>
    </div>
  `;
...
window.deleteCampaign = async function (id) {
  ...
  const c = AS.campaigns.find(x => x._id === id);
  if (!c) return;
  if (!confirm(`"${c.name}" kampanyası silinsin mi? Bu işlem geri alınamaz.`)) return;
  try {
...
window.viewDeliveryReport = async function (campaignId) {
  ...
  try {
    ...
    if (!deliveryData.length) {
      tbody.innerHTML = `<tr><td colspan="5" class="px-5 py-8 text-center text-sm text-gray-400">Henüz teslimat verisi yok</td></tr>`;
    } else {
      ...
      if (summary) summary.textContent = `${sent} gönderildi · ${failed} başarısız · ${replied} yanıtladı / ${total} toplam`;
    }
    ...
    showToast('Teslimat raporu yüklenemedi', 'error');
  }
};
...
window.retryCampaign = async function () {
  ...
  try {
    ...
    showToast('Başarısız göndermeler yeniden deneniyor…', 'info');
    closeDeliveryModal();
    loadCampaigns();
  } catch {
...
window.pauseCampaign = async function (id) {
  try { await apiFetch(`/crm/campaigns/${id}/pause`, 'PATCH'); showToast('Kampanya duraklatıldı', 'success'); loadCampaigns(); }
  catch { showToast('Kampanya duraklatılamadı', 'error'); }
};
window.resumeCampaign = async function (id) {
  try { await apiFetch(`/crm/campaigns/${id}/resume`, 'PATCH'); showToast('Kampanya devam ettirildi', 'success'); loadCampaigns(); }
  catch { showToast('Kampanya devam ettirilemedi', 'error'); }
};
window.cancelCampaign = async function (id) {
  if (!confirm('Bu kampanya iptal edilsin mi?')) return;
  try { await apiFetch(`/crm/campaigns/${id}/cancel`, 'PATCH'); showToast('Kampanya iptal edildi', 'success'); loadCampaigns(); }
  catch { showToast('Kampanya iptal edilemedi', 'error'); }
};
window.archiveCampaign = async function (id) {
  try { await apiFetch(`/crm/campaigns/${id}/archive`, 'PATCH'); showToast('Kampanya arşivlendi', 'success'); loadCampaigns(); }
  catch { showToast('Kampanya arşivlenemedi', 'error'); }
};
...
function renderAvailableContacts(contacts, preSelected = []) {
  ...
  el.innerHTML = '';
  if (!contacts.length) {
    el.innerHTML = '<p class="text-xs text-gray-400 text-center pt-8 px-4">Kişi bulunamadı</p>';
    return;
  }
  contacts.forEach(c => {
...
window.removeFromSelected = function (phone) {
  ...
  if (!document.querySelectorAll('#selected-contacts-list .sel-chip').length) {
    ...
    p.textContent = 'Henüz kişi seçilmedi';
