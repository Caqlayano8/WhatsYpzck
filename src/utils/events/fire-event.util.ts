/**
 * Author: Ç.Kurtoğlu
 * Description: Fire Event Utility - Olay tetikleme ve webhook
 */
...
function formatEmailSubject(event: string, payload: Record<string, any>): string {
    switch (event) {
        case 'message.received':   return `[WhatsYpzck] New message from ${payload.phoneNumber}`;
        case 'campaign.completed': return `[WhatsYpzck] Campaign "${payload.name}" completed`;
        case 'campaign.failed':    return `[WhatsYpzck] Campaign "${payload.name}" failed`;
        case 'contact.new':        return `[WhatsYpzck] New contact: ${payload.phoneNumber}`;
...
function formatEmailBody(event: string, payload: Record<string, any>): string {
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
      <div style="background:#075e54;padding:20px 24px;display:flex;align-items:center;gap:12px">
        ...
        <span style="color:#fff;font-size:1.1rem;font-weight:700">WhatsYpzck Notification</span>
      </div>
      <div style="padding:24px">
        ...
        <p style="margin:20px 0 0;font-size:0.78rem;color:#9ca3af">Sent by WhatsYpzck CRM · ${new Date().toLocaleString()}</p>
      </div>
    </div>`;
}
